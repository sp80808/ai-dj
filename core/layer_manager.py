...
class LayerManager:
    ...
    def _quantize_to_grid(self, audio, sr, layer_id, measures=1, quantize_level="16th"):
        """
        Force la quantization d'un sample sur la grille rythmique
        
        Args:
            audio: Signal audio à quantizer
            sr: Sample rate
            layer_id: ID du layer pour debug
            measures: Nombre de mesures target
            quantize_level: Niveau de quantization ("16th", "8th", "quarter")
        
        Returns:
            audio quantizé parfaitement sur la grille
        """
        print(f"🎯 Quantization grid pour '{layer_id}'...")
        ...
        target_length = samples_per_bar * measures
        
        # Grille de quantization
        quantize_divisions = {
            "16th": 16,  # 16ème de note
            "8th": 8,    # 8ème de note  
            "quarter": 4  # Noire
        }
        
        grid_resolution = quantize_divisions.get(quantize_level, 16)
        samples_per_grid = samples_per_beat // (grid_resolution // 4)
        
        print(f"   Grid: {grid_resolution} subdivisions, {samples_per_grid} samples par step")
        
        # 1. DÉTECTION DES KICKS/TRANSIENTS
        ...
        onsets = librosa.onset.onset_detect(
            ...
            units="samples",
            hop_length=512,
            delta=0.1  # Seuil plus élevé pour éviter les faux positifs
        )
        
        if len(onsets) == 0:
            print(f"⚠️  Aucun onset détecté pour '{layer_id}', quantization basique")
            return self._force_exact_length(audio, target_length)
        
        # 2. SNAP LES ONSETS SUR LA GRILLE
        quantized_onsets = []
        for onset in onsets[:8]:  # Max 8 onsets pour éviter le chaos
            # Trouver la position grid la plus proche
            grid_position = round(onset / samples_per_grid) * samples_per_grid
            
            # Éviter les doublons
            if not quantized_onsets or abs(grid_position - quantized_onsets[-1]) > samples_per_grid:
                quantized_onsets.append(int(grid_position))
                
            print(f"   Onset {onset/sr:.3f}s → Grid {grid_position/sr:.3f}s")
        
        # 3. RECONSTRUCTION QUANTIZÉE
        if len(quantized_onsets) >= 2:
            # Multi-onsets : découper et replacer sur la grille
            quantized_audio = self._reconstruct_with_grid_onsets(
                audio, sr, quantized_onsets, target_length, layer_id
            )
        else:
            # Un seul onset : alignement simple
            if quantized_onsets:
                shift = quantized_onsets[0]
                quantized_audio = self._align_single_onset(audio, shift, target_length)
            else:
                quantized_audio = self._force_exact_length(audio, target_length)
        
        print(f"✅ Quantization terminée pour '{layer_id}'")
        return quantized_audio

    def _reconstruct_with_grid_onsets(self, audio, sr, quantized_onsets, target_length, layer_id):
        """Reconstruit l'audio en plaçant les segments sur la grille"""
        
        print(f"🔧 Reconstruction avec {len(quantized_onsets)} onsets pour '{layer_id}'")
        
        # Buffer de sortie
        output_audio = np.zeros(target_length, dtype=audio.dtype)
        
        # Découper l'audio original en segments entre onsets
        segments = []
        prev_onset = 0
        
        for i, onset in enumerate(quantized_onsets):
            if i == 0:
                # Premier segment : du début au premier onset
                segment = audio[:onset] if onset < len(audio) else audio
            else:
                # Segments suivants
                start = prev_onset
                end = min(onset, len(audio))
                segment = audio[start:end] if start < len(audio) else np.array([])
            
            if len(segment) > 0:
                segments.append((segment, quantized_onsets[i] if i < len(quantized_onsets) else target_length))
            
            prev_onset = onset
        
        # Ajouter le segment final si nécessaire
        if prev_onset < len(audio):
            final_segment = audio[prev_onset:]
            segments.append((final_segment, target_length))
        
        # Placer les segments dans le buffer de sortie
        for segment, target_pos in segments:
            if target_pos >= target_length:
                break
                
            end_pos = min(target_pos + len(segment), target_length)
            actual_length = end_pos - target_pos
            
            if actual_length > 0:
                output_audio[target_pos:end_pos] = segment[:actual_length]
        
        return output_audio

    def _align_single_onset(self, audio, shift_samples, target_length):
        """Aligne un audio avec un seul onset principal"""
        
        # Créer le buffer de sortie
        output_audio = np.zeros(target_length, dtype=audio.dtype)
        
        # Calculer où placer l'audio
        if shift_samples >= 0 and shift_samples < target_length:
            # Longueur à copier
            copy_length = min(len(audio), target_length - shift_samples)
            
            # Copier l'audio à la position décalée
            output_audio[shift_samples:shift_samples + copy_length] = audio[:copy_length]
        
        return output_audio

    def _force_exact_length(self, audio, target_length):
        """Force un audio à avoir exactement la longueur target"""
        ...
        if len(audio) == target_length:
            ...
            if len(audio) > 0:
                ...
                # Audio vide : silence
                return np.zeros(target_length, dtype=audio.dtype)

    def _detect_rhythmic_pattern(self, audio, sr, layer_id):
        """
        Détecte le pattern rythmique dominant pour une quantization intelligente
        
        Returns:
            dict: Infos sur le pattern détecté
        """
        
        # Analyse spectrale pour détecter la périodicité
        onset_env = librosa.onset.onset_strength(y=audio, sr=sr)
        
        # Autocorrélation pour trouver la périodicité
        autocorr = np.correlate(onset_env, onset_env, mode='full')
        autocorr = autocorr[len(autocorr)//2:]
        
        # Chercher les pics de corrélation
        ...
        if len(peaks) > 0:
            ...
            period_seconds = period_frames * hop_length / sr
            
            # Déduire le type de pattern
            beat_duration = 60.0 / self.master_tempo
            
            if 0.9 * beat_duration < period_seconds < 1.1 * beat_duration:
                pattern_type = "quarter_note"
            elif 0.4 * beat_duration < period_seconds < 0.6 * beat_duration:
                pattern_type = "eighth_note"
            elif 0.2 * beat_duration < period_seconds < 0.3 * beat_duration:
                pattern_type = "sixteenth_note"
            else:
                pattern_type = "irregular"
                
            print(f"🎼 Pattern détecté pour '{layer_id}': {pattern_type} ({period_seconds:.3f}s)")
        ...
        return {"type": "unknown", "period_seconds": None, "confidence": 0.0}
