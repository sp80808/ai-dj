...
void DjIaVstProcessor::generateLoop(const DjIaClient::LoopRequest &request, const juce::String &targetTrackId)
{
	...
	try
	{
		if (!obsidianEngine)
		{
			setIsGenerating(false);
			setGeneratingTrackId("");
			notifyGenerationComplete(trackId, "ERROR: AI Engine not initialized. Please restart the plugin.");
			return;
		}

		ObsidianEngine::LoopRequest engineRequest;
		engineRequest.prompt = request.prompt;
		engineRequest.generationDuration = request.generationDuration;
		engineRequest.bpm = request.bpm;
		engineRequest.key = request.key;

		for (const auto &stem : request.preferredStems)
		{
			engineRequest.preferredStems.push_back(stem);
		}

		obsidianEngine->generateLoopAsync(engineRequest,
										  [this, trackId, request](ObsidianEngine::LoopResponse response)
										  {
											  juce::MessageManager::callAsync([this, trackId, request, response]()
																			  { handleGenerationComplete(trackId, request, response); });
										  });
